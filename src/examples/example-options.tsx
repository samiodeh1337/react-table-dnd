...
const thBase: React.CSSProperties = {
  ...
  fontSize: 12, fontWeight: 700, borderBottom: "2px solid #3b2d6e",
...
const tdBase: React.CSSProperties = {
  ...
  borderBottom: "1px solid #1e1836", display: "flex", alignItems: "center",
};
const tdLocked: React.CSSProperties = { ...tdBase, background: "#140f26", color: "#a78bfa", fontStyle: "italic" };
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      <p style={{ margin: "0 0 12px", fontSize: 12, color: "#8b8b94" }}>
        First {LOCKED_COLS} col locked | First {LOCKED_ROWS_START} rows &amp; last {LOCKED_ROWS_END} rows locked (italic/purple = locked)
      </p>
      <TableContainer
        options={options} onDragEnd={handleDragEnd}
        renderPlaceholder={() => (
          <div style={{ width: "100%", height: "100%", background: "#1a1230", border: "2px dashed #a78bfa", borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#a78bfa", fontWeight: 600 }}>
            Drop here
          </div>
        )}
      ...
      >
        <TableHeader>
          {cols.map((col, i) => (
            <ColumnCell key={col.id} id={col.id} index={i} width={col.width} style={isColLocked(i) ? thLocked : thNormal}>
